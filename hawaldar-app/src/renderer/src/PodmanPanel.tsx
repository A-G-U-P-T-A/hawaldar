...
import type { PodmanSetupProgress, PodmanStatusDTO, PodmanServiceDTO, RuntimeStateDTO } from '../../preload/api';
...
function bannerTitle(status: PodmanStatusDTO): string {
	...
	switch (status.availability) {
		...
		case 'not_installed':
			return docker ? 'Docker CLI not found' : 'Podman is required';
		case 'no_machine':
...
function bannerDetail(status: PodmanStatusDTO): string {
	...
	if (status.availability === 'not_installed') {
		return 'Tools do not run until you Set up Podman. That is the in-app install path.';
	}
...
function groupServices(services: PodmanServiceDTO[]) {
	const groups: Array<{ key: string; label: string; webLab: boolean; optional: boolean; items: PodmanServiceDTO[] }> = [];
	for (const service of services) {
		const key = service.lane || 'other';
		...
		groups.push({
			key,
			label: service.laneLabel || key,
			webLab: Boolean(service.webLab),
			optional: key === 'web-optional',
			items: [service],
		});
	}
	return groups;
	...
	if (!status && !cached) {
		return (
			<PageShell
				...
				actions={<button type="button" className="btn" onClick={onClose}>{t('settings.backToChat')}</button>}
			>
				<section className="widget">
					<p className="widget-help">{t('runtime.loading')}</p>
				</section>
			</PageShell>
		);
	}
		...
		>
			<section className="widget">
				...
				<RuntimeActions
					...
					busy={Boolean(busy)}
					busyKey={busy}
					setupLabel={docker && clientMissing ? t('runtime.setupInstead') : t('runtime.setup')}
					onSetup={runSetup}
					onStart={() => runMachine('start')}
					onStop={() => runMachine('stop')}
					onRestart={() => runMachine('restart')}
					autoStart={Boolean(status?.autoStartMachine)}
					onAutoStart={(enabled) => {
						void withBusy('auto', async () => {
							applyStatus(await window.hawaldar.setAutoStartMachine(enabled));
						});
					}}
				/>
			</section>
			...
			<section className="widget">
				...
				<p className="widget-help">
					Default off. Toggle to build or pull that image. Juice Shop web PoC: juice-shop, httpx, katana, zap, sqlmap, browser (poc/research share it), scrapling.
					Leave ghidra, radare, binwalk, tshark, subfinder, and amass off — they are not used for a localhost web lab.
				</p>
				<div className="table-wrap">
					<table className="data-table">
						<colgroup>
						...
						<tbody>
							...
							{groupServices(status.services).map((group) => (
