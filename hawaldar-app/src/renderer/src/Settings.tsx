...
function groupReadiness(items: ReadinessCheckDTO[]) {
	const groups: Array<{ key: string; label: string; hint: string; webLab: boolean; items: ReadinessCheckDTO[] }> = [];
...
function matchesToolQuery(query: string, ...parts: Array<string | undefined>): boolean {
...
export default function Settings({ onSaved, onClose, initialCategory = 'provider', onCategoryChange }: Props) {
	const { t } = useI18n();
	...
	const settingsSplitRef = useRef<HTMLDivElement>(null);
	const settingsNav = usePersistedPanelSize('settingsNav');
	const settingsNavResize = useDragResize({
		getValue: () => settingsNav.sizeRef.current,
		apply: settingsNav.setSize,
		clamp: (next) => {
			const width = settingsSplitRef.current?.clientWidth ?? 800;
			return clampGrow(next, settingsNav.min, width - MAIN_PANE_MIN);
		},
		onCommit: settingsNav.commit,
	});
	...
	const refreshModels = useCallback(async (
		...
		try {
			...
			if (!result.models.some(isListedFree)) {
				setFreeOnly(false);
			}
			if (result.error) {
	...
	const applyRuntimeStatus = (next: PodmanStatusDTO) => {
		...
		setForm((prev) => prev
			? {
				...
				host: next.host,
				autoStartMachine: next.autoStartMachine,
			}
			: prev);
	};
	...
	useEffect(() => {
		void (async () => {
			...
			if (s.hasSelectedProvider && s.provider) {
	...
	const filteredBuiltin = useMemo(
		() => TOOL_CATALOG.filter((tool) => matchesToolQuery(toolsQuery, tool.title, tool.id, tool.agentId)),
		[toolsQuery],
	);
	const filteredCustom = useMemo(
		() => (form?.customTools ?? []).filter((tool) => (
			matchesToolQuery(toolsQuery, tool.title, tool.id, tool.agentId, tool.command)
		)),
		[form?.customTools, toolsQuery],
	);

	const selectedProvider = form ? providers.find((p) => p.id === form.provider) : undefined;
	const selectedListed = form ? findListedModel(models, form.model) : undefined;
	const showThinking = selectedListed?.supportsReasoning === true;
	const hasFreeModels = models.some(isListedFree);
	const pickerModels = freeOnly && hasFreeModels ? models.filter(isListedFree) : models;
	const categoryLabel = t(CATEGORIES.find((item) => item.id === category)?.labelKey || 'settings.title');
	...
	const setToolsEnabled = (ids: string[], enabled: boolean) => {
		if (ids.length === 0) {
			return;
		}
		...
		setForm((prev) => {
			if (!prev) return prev;
			const next = enabled
				? [...new Set([...prev.enabledTools, ...ids])]
				: prev.enabledTools.filter((id) => !idSet.has(id));
	...
	const save = async () => {
		...
		try {
			await window.hawaldar.saveSettings({
				...
				customTools: form.customTools,
				containerEngine: form.containerEngine,
				autoStartMachine: form.autoStartMachine,
	...
	const widgetFoot = (extra?: ReactNode, saveDisabled = false) => (
		<div className="widget-foot">
			{extra}
			<button type="button" className="btn btn-primary" disabled={!form || saving || saveDisabled} onClick={() => void save()}>
				{saving ? t('settings.saving') : t('settings.save')}
			</button>
			<button type="button" className="btn" onClick={onClose}>{t('settings.cancel')}</button>
		</div>
	);
	...
	const settingsFrame = (main: ReactNode) => (
		<div
			ref={settingsSplitRef}
			className={`settings-layout${settingsNavResize.active ? ' is-resizing' : ''}`}
			style={{ ['--settings-nav-w' as string]: `${settingsNav.size}px` }}
		>
			{nav}
			<PaneSash
				className="settings-sash"
				label="Settings category list"
				active={settingsNavResize.active}
				onPointerDown={settingsNavResize.onPointerDown}
				onDoubleClick={settingsNav.reset}
			/>
			{main}
		</div>
	);
			...
			<div className="settings-main">
				...
				<div className="settings-main-body">
					<section className="widget">
						<p className="widget-help">{t('settings.loading')}</p>
					</section>
				</div>
			</div>,
		);
	...
	if (category === 'provider') {
		body = (
			<section className="widget">
				<div className="widget-head">
					<h2 className="widget-title">{t('settings.widget.provider')}</h2>
				</div>
					...
					<div className="field">
						...
						<Dropdown
							...
							options={providers.map((p) => ({
								value: p.id,
								label: form.hasSelectedProvider && p.id === form.provider ? `${p.label} · active` : p.label,
				...
				{widgetFoot(
					<>
						...
						<button
							type="button"
							className="btn"
							disabled={modelsLoading || !form.provider}
							onClick={() => void refreshModels(form.provider, form.baseUrl, apiKey || undefined, form.model, true)}
						>
		...
		body = (
			<section className="widget">
				<div className="widget-head">
					<h2 className="widget-title">{t('settings.widget.scope')}</h2>
				</div>
				<p className="widget-help">One host, domain, or CIDR per line. Prefix ! to deny. If you say “scan ports” with no host, Hawaldar uses this list.</p>
				<textarea
					className="mono-input"
					rows={12}
					value={scopeText}
		...
		const runAutoStart = async (enabled: boolean) => {
			setRuntimeBusy('autostart');
			setPodmanOut('');
			try {
				applyRuntimeStatus(await window.hawaldar.setAutoStartMachine(enabled));
				setForm((prev) => prev ? { ...prev, autoStartMachine: enabled } : prev);
			} catch (error) {
				setPodmanOut(error instanceof Error ? error.message : String(error));
			} finally {
				setRuntimeBusy(null);
		...
		body = (
			<section className="widget">
				<div className="widget-head">
					<h2 className="widget-title">{t('settings.widget.runtime')}</h2>
				...
				{runtimeView.phase !== 'setup' && (
					<details className="runtime-advanced">
						...
						<div className="field">
							<label htmlFor="settings-session-ttl">Delete unpinned sessions after (days)</label>
							<input
								id="settings-session-ttl"
								type="number"
								min={30}
								max={3650}
								value={form.sessionTtlDays ?? 365}
								onChange={(e) => setForm({ ...form, sessionTtlDays: Number(e.target.value) })}
								disabled={runtimeBusyAny}
							/>
						</div>
						<div className="widget-foot">
						...
						<>
							<RuntimeActions
								...
								setupLabel={form.containerEngine === 'docker' && !dockerDetected
									? t('runtime.setupInstead')
									: form.containerEngine === 'docker'
										? t('runtime.checkDocker')
										: t('runtime.setup')}
								onSetup={() => void runSetup()}
								onStart={() => void runMachine('start')}
								onStop={() => void runMachine('stop')}
								onRestart={() => void runMachine('restart')}
								autoStart={Boolean(runtimeStatus?.autoStartMachine ?? form.autoStartMachine)}
								onAutoStart={(enabled) => void runAutoStart(enabled)}
							/>
							{widgetFoot()}
						</>
					)}
		...
		body = (
			<section className="widget">
				<div className="widget-head">
					<h2 className="widget-title">{t('settings.widget.builtin')}</h2>
				</div>
				<p className="widget-help">Enable tools and override the agent image. Hawaldar tools run in Podman images. New catalog tools default on.</p>
				<div className="tools-toolbar">
					<input
						type="search"
						value={toolsQuery}
						onChange={(e) => setToolsQuery(e.target.value)}
						placeholder="Search name, id, or agent…"
						aria-label="Search built-in tools"
					/>
					<button
						type="button"
						className="btn"
						disabled={filteredBuiltin.length === 0}
						onClick={() => setToolsEnabled(filteredBuiltin.map((tool) => tool.id), true)}
					>
						Enable all
					</button>
					<button
						...
						className="btn"
						disabled={filteredBuiltin.length === 0}
						onClick={() => setToolsEnabled(filteredBuiltin.map((tool) => tool.id), false)}
					>
						Disable all
					</button>
				</div>
					...
					<table className="data-table tools-table">
						...
						<tbody>
							{filteredBuiltin.length === 0 && (
								<tr>
									<td colSpan={5} className="table-empty">No matching tools.</td>
								</tr>
							)}
							{filteredBuiltin.map((tool) => (
								<tr key={tool.id}>
									...
									<td>
										<input
											className="mono-input"
											value={form.toolImages[tool.agentId]?.trim() || tool.image}
		...
		body = (
			<section className="widget">
				<div className="widget-head">
					<h2 className="widget-title">{t('settings.widget.custom')}</h2>
				</div>
				<p className="widget-help">
				...
				<div className="tools-toolbar">
					...
					<button
						type="button"
						className="btn"
						disabled={filteredCustom.length === 0}
						onClick={() => setToolsEnabled(filteredCustom.map((tool) => tool.id), true)}
					>
						Enable all
					</button>
					<button
						...
						className="btn"
						disabled={filteredCustom.length === 0}
						onClick={() => setToolsEnabled(filteredCustom.map((tool) => tool.id), false)}
					>
						Disable all
					</button>
				</div>
				<div className="table-wrap">
					<table className="data-table tools-table custom-tools-table">
						...
						<tbody>
							...
							{form.customTools.length > 0 && filteredCustom.length === 0 && (
								<tr>
									<td colSpan={6} className="table-empty">No matching tools.</td>
								</tr>
							)}
		...
		body = (
			<section className="widget">
				<div className="widget-head">
					<h2 className="widget-title">{t('settings.widget.prompts')}</h2>
				</div>
				<p className="widget-help">
		...
		body = (
			<section className="widget">
				<div className="widget-head">
					<h2 className="widget-title">{t('settings.widget.readiness')}</h2>
				</div>
				<p className="widget-help">
					Juice Shop / localhost web PoC needs the Web lab group (httpx, katana, zap, sqlmap, browser, scrapling, juice-shop).
					Binary, pcap, and subdomain images can stay off — they are not required.
				</p>
				...
				<div className="table-wrap">
					<table className="data-table readiness-table">
						...
						<tbody>
							...
							{groupReadiness(readiness).map((group) => (
								<Fragment key={group.key}>
									<tr className="readiness-group">
										<td colSpan={3}>
											<strong>{group.label}</strong>
											{group.key !== 'engine' && (
												<span className="readiness-group-tag">
													{group.webLab
														? 'needed for Juice Shop'
														: group.key === 'web-optional'
															? 'optional for Juice Shop'
															: 'not required for a localhost web lab'}
												</span>
											)}
											{group.hint && <div className="widget-help">{group.hint}</div>}
										</td>
									</tr>
		...
		<div className="settings-main">
				<div className="settings-main-head">
					<div className="settings-main-heading">
						...
						{category === 'tools' && (
							<div className="settings-subnav" role="tablist" aria-label={t('settings.category.tools')}>
								<button
									type="button"
									role="tab"
									aria-selected={toolsSub === 'builtin'}
									className={toolsSub === 'builtin' ? 'active' : ''}
									onClick={() => { setToolsSub('builtin'); setToolsQuery(''); }}
								>
								...
								<button
									...
									onClick={() => { setToolsSub('custom'); setToolsQuery(''); }}
								>
