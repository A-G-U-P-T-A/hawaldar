...
export function runCommand(
	...
	return new Promise((resolve, reject) => {
		...
		const timer = setTimeout(() => {
			settled = true;
			child.kill('SIGKILL');
			resolve(classifyResult({ exitCode: 124, stdout, stderr, timedOut: true }));
		}, timeoutMs);
		...
		child.on('error', (error) => {
			...
			clearTimeout(timer);
			reject(new Error(classifySpawnError(error)));
		});
		child.on('close', (code) => {
			if (settled) {
				return;
			...
			clearTimeout(timer);
			resolve(classifyResult({ exitCode: code ?? 1, stdout, stderr, timedOut: false }));
		});
	});
}

function classifyResult(result: CommandResult): CommandResult {
	if (!isStaleNetworkBackend(`${result.stdout}\n${result.stderr}`)) {
		return result;
	}
	return { ...result, stderr: STALE_NETWORK_BACKEND };
}
