...
import { resolveWorkflowRef, stepLooksLikeExploit } from './engagement';
...
import { AGENT_ROLES, isServiceControlTool, TOOL_CATALOG } from './tools/catalog';
...
const BUILTIN_WORKFLOWS: WorkflowRecord[] = [
	...
	},
	{
		...
		name: 'Web recon',
		steps: [
			{ kind: 'tool', id: 'httpx-tech' },
			{ kind: 'tool', id: 'katana' },
			{ kind: 'tool', id: 'scrapling-fetch' },
			{ kind: 'tool', id: 'scrapling-links' },
			{ kind: 'tool', id: 'browser-open' },
			{ kind: 'tool', id: 'browser-links' },
		],
		enabled: true,
		builtin: true,
		updatedAt: 0,
	},
	{
		...
		name: 'ZAP web scan',
		steps: [
			{ kind: 'tool', id: 'zap-status' },
			{ kind: 'tool', id: 'zap-spider' },
			{ kind: 'tool', id: 'zap-pscan' },
		],
		enabled: true,
		builtin: true,
		updatedAt: 0,
	...
	},
	{
		...
		steps: [
			{ kind: 'workflow', id: 'pre-recon' },
			{ kind: 'workflow', id: 'recon-surface' },
			{ kind: 'workflow', id: 'vuln-detect' },
			{ kind: 'workflow', id: 'poc-validate' },
			{ kind: 'workflow', id: 'validate' },
			{ kind: 'workflow', id: 'report' },
		],
		enabled: true,
		builtin: true,
		updatedAt: 0,
...
export function evaluateToolRules(
	...
	for (const rule of rules) {
		...
		if (rule.kind === 'require_service' && !isServiceControlTool(toolId)) {
...
function agentForTool(settings: HawaldarSettings, toolId: string): string | undefined {
	if (isServiceControlTool(toolId)) {
		return undefined;
	}
...
export function assertSafeSteps(
	steps: WorkflowStep[],
	settings?: HawaldarSettings,
	opts?: { workflowId?: string; knownWorkflowIds?: string[] },
): string | undefined {
	if (!Array.isArray(steps) || steps.length === 0) {
		return 'Add at least one tool, agent, or workflow step.';
	}
	if (steps.length > 20) {
		return 'A workflow can have at most 20 steps.';
	}
	...
	const knownWorkflows = new Set([
		...BUILTIN_WORKFLOWS.map((item) => item.id),
		...(opts?.knownWorkflowIds || []),
	]);
	for (const step of steps) {
		if (!step || !STEP_KINDS.has(step.kind)) {
			return 'Each step must be a tool, agent, or workflow.';
		}
		if (!ID_RE.test(step.id) && !/^[a-z][a-z0-9_]*$/i.test(step.id)) {
			return `Invalid step id: ${step.id}`;
		}
		if (FORBIDDEN_STEP.test(step.id) || stepLooksLikeExploit(step.id) || rejectForbiddenTool(step.id)) {
			return `${step.id} is outside Hawaldar policy. Exploit tooling stays refused; proof runs through the sanctioned poc-* validators.`;
		}
		...
		if (step.kind === 'workflow') {
			if (opts?.workflowId && (step.id === opts.workflowId || resolveWorkflowRef(step.id) === opts.workflowId)) {
				return 'A workflow cannot nest itself.';
			}
			if (knownWorkflows.size > 0 && !knownWorkflows.has(step.id) && !knownWorkflows.has(resolveWorkflowRef(step.id))) {
				return `Unknown workflow: ${step.id}`;
			}
		}
	}
	return undefined;
...
export function workflowHasExploitStep(steps: WorkflowStep[]): boolean {
	return steps.some((step) => FORBIDDEN_STEP.test(step.id) || stepLooksLikeExploit(step.id));
}
...
export class PlaybookStore {
	...
	getWorkflow(keyOrId: string): WorkflowRecord | undefined {
		const found = findWorkflow(this.workflows, resolveWorkflowRef(keyOrId));
	...
	async upsertWorkflow(draft: WorkflowWrite, settings?: HawaldarSettings): Promise<WorkflowRecord> {
		...
		const stepError = assertSafeSteps(draft.steps, settings, {
			workflowId: draft.id,
			knownWorkflowIds: this.workflows.map((item) => item.id),
		});
		if (stepError) {
	...
	private async init(): Promise<void> {
		...
		for (const seed of BUILTIN_WORKFLOWS) {
			...
			// Builtins track the shipped definition: sync name + steps, keep the operator's enabled flag.
			const current = existing.find((item) => item.id === seed.id);
			if (current && (current.name !== seed.name || JSON.stringify(current.steps) !== JSON.stringify(seed.steps))) {
				await this.client.execute({
					sql: 'UPDATE workflows SET name = ?, steps = ?, updatedAt = ? WHERE id = ?',
					args: [seed.name, JSON.stringify(seed.steps), now, seed.id],
				});
			}
		}
		this.workflows = await this.readWorkflows();
...
function parseSteps(raw: unknown): WorkflowStep[] {
	try {
		...
		return parsed
			.filter((item) => item && STEP_KINDS.has(item.kind) && typeof item.id === 'string')
