	...
	return (
		<section className="widget">
			...
			<p className="widget-help">
				Stored in <code>~/.hawaldar/hawaldar.db</code>. Ordered tool/agent steps. Policy still owns scope; rules apply at run time.
			</p>
							...
							<div key={`${step.kind}-${index}`} className="step-row">
								<Dropdown
									...
									onChange={(kind) => {
										const next = [...steps];
										next[index] = {
											kind: kind as WorkflowStepDTO['kind'],
											id: kind === 'agent' ? (agentOptions[0]?.value || 'nmap') : (toolOptions[0]?.value || 'quick-scan'),
										};
										setSteps(next);
									}}
								/>
								<Dropdown
									...
									value={step.id}
									options={step.kind === 'agent' ? agentOptions : toolOptions}
									onChange={(id) => {
										const next = [...steps];
										next[index] = { ...step, id };
										setSteps(next);
